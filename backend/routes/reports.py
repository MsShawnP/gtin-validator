...
def download_csv_report(token: str, company_name: str = "") -> Response:
    ...
    filename = f"{company_name.replace(' ', '_')}_report.csv" if company_name else "gtin_report.csv"
    return Response(
        content=csv_string,
        media_type="text/csv",
...
def download_corrected_csv(token: str, company_name: str = "") -> Response:
    ...
    filename = f"{company_name.replace(' ', '_')}_corrected.csv" if company_name else "gtin_corrected.csv"
    return Response(
        content=csv_string,
        media_type="text/csv",
        headers={"Content-Disposition": f'attachment; filename="{filename}"'},
...
def download_pdf_report(token: str, company_name: str = "") -> StreamingResponse:
    ...
    filename = f"{company_name.replace(' ', '_')}_report.pdf" if company_name else "gtin_report.pdf"
